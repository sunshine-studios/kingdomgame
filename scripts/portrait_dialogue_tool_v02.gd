...
@export var json_file_path: String = "res://jsons/choice_system_example.json"
@export var scene_dialogue_sequence: Array[String] = ["player_01","kingdoma_01","player_02","kingdoma_02", "choice_moment", "player_response"]
...
# Choice system references
@onready var choice_box = $UILayer/ChoiceBox
@onready var choice_prompt = $UILayer/ChoiceBox/ChoiceContainer/ChoicePrompt
@onready var choice1_button = $UILayer/ChoiceBox/ChoiceContainer/Choice1Button
@onready var choice2_button = $UILayer/ChoiceBox/ChoiceContainer/Choice2Button
@onready var choice3_button = $UILayer/ChoiceBox/ChoiceContainer/Choice3Button
...
func _ready():
	...
	# Hide all portraits and choice box initially
	hide_all_portraits()
...
func setup_character_portraits():
	...
	for child in get_children():
		if child.name.ends_with("PortraitStandIn"):
			var node_name = child.name.replace("PortraitStandIn", "").to_lower()
			var character_name = node_name
			
			# Map old node names to new character names
			if node_name == "merlin":
				character_name = "player"
			elif node_name == "luna":
				character_name = "kingdoma"
			elif node_name == "sierra":
				character_name = "kingdomb"
			elif node_name == "annabell":
				character_name = "kingdomc"
...
func display_dialogue_line(dialogue_id: String):
	...
	# Check if there's a choice at this index
	var choice_key = "choice_" + str(current_line_index)
	if choice_key in dialogue_lines:
		display_choice(dialogue_lines[choice_key])
		return
...
func update_portrait(dialogue_id: String, expression_path: String):
	...
	if dialogue_id.begins_with("player"):
		...
		character_name = "kingdoma"
	elif dialogue_id.begins_with("kingdomb"):
		character_name = "kingdomb"
	elif dialogue_id.begins_with("kingdomc"):
		character_name = "kingdomc"
	else:
...
func _on_dialogue_button_pressed():
	# Don't process if waiting for choice
...
func display_choice(choice_data: Dictionary):
	print("Displaying choice: ", choice_data)
	current_choice_data = choice_data
	waiting_for_choice = true
	is_choice_active = true
	
	# Hide dialogue box and show choice box
	dialogue_box.visible = false
	choice_box.visible = true
	
	# Set choice prompt text
	if "prompt" in choice_data:
		choice_prompt.text = choice_data["prompt"]
	else:
		choice_prompt.text = "What do you choose?"
	
	# Configure choice buttons
	var choices = choice_data.get("options", [])
	var choice_buttons = [choice1_button, choice2_button, choice3_button]
	
	# Show/hide buttons based on number of choices
	for i in range(choice_buttons.size()):
		if i < choices.size():
			choice_buttons[i].visible = true
			choice_buttons[i].text = choices[i].get("text", "Choice " + str(i + 1))
		else:
			choice_buttons[i].visible = false

func _on_choice_selected(choice_index: int):
	if not is_choice_active or choice_index >= current_choice_data.get("options", []).size():
		return
		
	var selected_choice = current_choice_data["options"][choice_index]
	print("Choice selected: ", selected_choice)
	
	# Apply global variable changes if any
	if "dominance_change" in selected_choice:
		GameState.modify_dominance(selected_choice["dominance_change"])
		print("Dominance modified by: ", selected_choice["dominance_change"])
	
	if "reputation_change" in selected_choice:
		GameState.modify_reputation(selected_choice["reputation_change"])
		print("Reputation modified by: ", selected_choice["reputation_change"])
	
	# Handle next dialogue
	if "next_dialogue" in selected_choice:
		# Change to a different dialogue sequence
		var next_dialogue_id = selected_choice["next_dialogue"]
		# Find the dialogue in the sequence or add it
		current_dialogue_index = scene_dialogue_sequence.find(next_dialogue_id)
		if current_dialogue_index == -1:
			# Add the new dialogue to sequence temporarily
			scene_dialogue_sequence.append(next_dialogue_id)
			current_dialogue_index = scene_dialogue_sequence.size() - 1
		current_line_index = 0
	elif "skip_to_dialogue" in selected_choice:
		# Skip to a specific dialogue index
		var skip_index = selected_choice["skip_to_dialogue"]
		if skip_index < scene_dialogue_sequence.size():
			current_dialogue_index = skip_index
			current_line_index = 0
	else:
